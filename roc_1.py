...
df_test = pd.read_csv("payloads/SQLi_1/TF_IDF/test.csv")
X_test = df_test.loc[:, df_test.columns != 'Label']
y_test = df_test['Label']

clf = pickle.load(open("models/SQLi_1_gbc_0.1_1000_4.sav", 'rb'))

test_pred_grad = clf.predict_proba(X_test)[:,1]
test_pred_grad_o = clf.predict(X_test)
print(f1_score(y_test, test_pred_grad_o))

clf1 = pickle.load(open("models/SQLi_1_lr_newton-cg_10.sav", 'rb'))
...
model = keras.models.load_model("models/SQLi_1_nn_0.001_64_sigmoid.h5")
...
plt.savefig('roc_curves/SQLi_1.png', dpi=2000, bbox_inches='tight')
plt.show()
